...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('daily-roll')
        .setDescription('Roll for random cards and pick one (24h cooldown)'),
    async execute(interaction) {
        const userId = interaction.user.id;
        
        if (db.hasActiveRoll(userId)) {
            return interaction.reply({
                content: 'You have an active roll in progress. Please complete it first.',
                flags: MessageFlags.Ephemeral
            });
        }

        if (!db.canRoll(userId, 'daily')) {
            const timeRemaining = db.getTimeUntilNextRoll(userId, 'daily');
            const formattedTime = db.formatTime(timeRemaining);
            
            const embed = new EmbedBuilder()
                .setColor(0xFF0000)
                .setTitle('Cooldown Active')
                .setDescription(`You've already used your daily roll. Please wait **${formattedTime}** before trying again.`);
                
            return interaction.reply({ embeds: [embed], flags: MessageFlags.Ephemeral });
        }

        db.setRollActive(userId, 'daily');

        await interaction.deferReply();

        try {
            const options = getRandomCardOptions('daily', 3);
            ...
            await interaction.editReply({
                content: '**Daily Roll** - Browse and select a card to keep:',
                embeds: [embed],
                components: [navRow],
                files: files
            });
            ...
            collector.on('end', async (collected, reason) => {
                try {
                    ...
                    console.error('Error in dailyroll collector end:', error);
                } finally {
                    db.clearActiveRoll(userId);
                }
            });
        } catch (error) {
            console.error('Error in dailyroll command:', error);
            db.clearActiveRoll(userId);
            if (!interaction.replied && !interaction.deferred) {
                return interaction.reply({
                    content: 'An error occurred while processing your roll. Please try again.',
                    flags: MessageFlags.Ephemeral
                });
            }
