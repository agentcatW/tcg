...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('hourly-roll')
        .setDescription('Roll for random cards and pick one (1h cooldown)'),
    async execute(interaction) {
        ...
        if (!db.canRoll(userId, 'hourly')) {
            const timeRemaining = db.getTimeUntilNextRoll(userId, 'hourly');
            const formattedTime = db.formatTime(timeRemaining);
            
            const embed = new EmbedBuilder()
                .setColor(0xFF0000)
                .setTitle('Cooldown Active')
                .setDescription(`You've already used your hourly roll. Please wait **${formattedTime}** before trying again.`);
                
            return interaction.reply({ embeds: [embed], flags: MessageFlags.Ephemeral });
        }

        db.setRollActive(userId, 'hourly');

        await interaction.deferReply();

        try {
            const options = getRandomCardOptions('hourly', 3);
            ...
            await interaction.editReply({
                content: '**Hourly Roll** - Browse and select a card to keep:',
                embeds: [embed],
                components: [navRow],
                files: files
            });
            ...
            collector.on('end', async (collected, reason) => {
                try {
                    ...
                    console.error('Error in hourlyroll collector end:', error);
                } finally {
            ...
            console.error('Error in hourlyroll command:', error);
            await interaction.editReply({ 
